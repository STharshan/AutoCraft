    ...
    <section className="bg-white py-20 px-4 text-center">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-6 max-w-6xl mx-auto">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="w-72 h-72 bg-gray-50 border border-gray-200 rounded-md shadow-sm hover:shadow-md transition duration-300 flex flex-col items-center justify-center"
          >
            <div className="text-4xl sm:text-5xl font-bold text-lime-300">{stat.value}</div>
            <div className="text-sm text-gray-500 mt-4">{stat.label}</div>
          </div>
        ))}
      </div>
    </section>
